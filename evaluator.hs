...
add stack = let (val1, stackAfterPop1) = pop stack
            in case val1 of
                ...
                Just (Number i1) ->
                    ...
                    in case val2 of
                        Nothing -> stack
                        Just (Number i2) -> pushToStack (Number (i1 + i2)) stackAfterPop2
                        _ -> stack
                _ -> stack
...
eq stack = let (val1, stackAfterPop1) = pop stack
           in case val1 of
               ...
               Just (Number i1) ->
                   ...
                   in case val2 of
                       Nothing -> stack
                       Just (Number i2) -> if i1 == i2 then pushToStack TT stackAfterPop2 else pushToStack FF stackAfterPop2
                       _ -> stack
               Just TT ->
                   let (val2, stackAfterPop2) = pop stackAfterPop1
                   in case val2 of
...
run (instruction:remainingCode, stack, state) =
    case instruction of
        ...
        And -> run (remainingCode, Evaluator.and stack, state)
        Neg -> run (remainingCode, Evaluator.neg stack, state)
